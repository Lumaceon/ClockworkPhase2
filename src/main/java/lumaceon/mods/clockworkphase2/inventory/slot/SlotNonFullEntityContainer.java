...
public class SlotNonFullEntityContainer extends Slot
{
    @CapabilityInject(IEntityContainer.class)
    public static final Capability<IEntityContainer> ENTITY_CONTAINER = null;

    public SlotNonFullEntityContainer(IInventory inventoryIn, int index, int xPosition, int yPosition) {
    ...
    public boolean isItemValid(ItemStack is)
    {
        if(!is.isEmpty() && is.hasCapability(ENTITY_CONTAINER, EnumFacing.DOWN))
        {
            ...
            if(container != null && container.getCapacity() > container.getEntityCount())
            {
                return true;
            }
        }
