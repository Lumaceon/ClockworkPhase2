...
public class EntityTemporalFishHook extends EntityFishHook
{
    ...
    private EntityTemporalFishHook.State currentState = EntityTemporalFishHook.State.FLYING;
    private int luck;
    private int lureSpeed;

    @SideOnly(Side.CLIENT)
    public EntityTemporalFishHook(World worldIn, EntityPlayer p_i47290_2_, double x, double y, double z)
    {
    ...
    public EntityTemporalFishHook(World worldIn, EntityPlayer fishingPlayer)
    {
    ...
    public void onUpdate()
    {
        ...
        else if (this.world.isRemote || !this.shouldStopFishing())
        {
            ...
            if (this.currentState == EntityTemporalFishHook.State.FLYING)
            {
                if (this.caughtEntity != null)
                {
                    ...
                    this.motionY = 0.0D;
                    this.motionZ = 0.0D;
                    this.currentState = EntityTemporalFishHook.State.HOOKED_IN_ENTITY;
                    return;
                }
                ...
                if (f > 0.0F)
                {
                    ...
                    this.motionY *= 0.2D;
                    this.motionZ *= 0.3D;
                    this.currentState = EntityTemporalFishHook.State.BOBBING;
                    return;
                }
            ...
            {
                if (this.currentState == EntityTemporalFishHook.State.HOOKED_IN_ENTITY)
                {
                    if (this.caughtEntity != null)
                    {
                        if (this.caughtEntity.isDead)
                        {
                            ...
                            this.currentState = EntityTemporalFishHook.State.FLYING;
                        }
                        else
                        {
                ...
                if (this.currentState == EntityTemporalFishHook.State.BOBBING)
                {
                    this.motionX *= 0.9D;
                    this.motionZ *= 0.9D;
    ...
    public int handleHookRetraction()
    {
        if (!this.world.isRemote && this.angler != null)
        {
            ...
            {
                boolean armillaryCatch = false;
                List<ItemStack> result = null;
                IBlockState blockState = world.getBlockState(this.getPosition());
                IBlockState blockStateBelow = world.getBlockState(this.getPosition().down());

                if(blockState != null && blockState.getBlock() != null && blockState.getBlock().equals(ModFluids.TIMESTREAM.getBlock()))
                {
                    armillaryCatch = true;
                }
                else if(blockStateBelow != null && blockStateBelow.getBlock() != null && blockStateBelow.getBlock().equals(ModFluids.TIMESTREAM.getBlock()))
                {
                    armillaryCatch = true;
                }
